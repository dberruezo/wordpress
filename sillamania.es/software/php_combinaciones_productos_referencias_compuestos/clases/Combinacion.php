<?php
/*
 *  1.- En esta clase cogemos la plantilla de Raul de tablas sillamania,
 *  leemos toda la columna de colores excepto LA ROJA Y LA AMARILLA
 *  y luego cambiamos algun tipo de color como en femenino y tipos no 
 *  conocidos como plata, plomo etc y los cambiamos ... Luego creamos 
 *  el fichero de combinaciones con el atributo de color por el momento
 * 
 *  2.- Como tenemos las referencias de productos compuestos por eso 
 *  estan en combinaciones, pues entonces creamos un csv de productos
 *  solo para poner la PRIMERA referemcia de producto compuesto.
 */
...
class Combinacion{
    ...
    public function create_excel(){
        ...
        // Excel prouductos para insertar la referencia de un producto compuesto 
        $fileName = realpath(dirname(__FILE__) . './../products_import_referencias_compuestos.xls');
        $fileType = 'Excel5';
        $objReader = PHPExcel_IOFactory::createReader($fileType);
        $objPHPExcel_products = $objReader->load($fileName);
        $objPHPExcel_products->getProperties()->setCreator("David Berruezo")
        ...
        $objPHPExcel_products->getActiveSheet()->getPageSetup()->setOrientation(PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE);
        $objPHPExcel_products->getActiveSheet()->getPageSetup()->setPaperSize(PHPExcel_Worksheet_PageSetup::PAPERSIZE_A4);
        ...
        $objPHPExcel_products->getActiveSheet()->getPageSetup()->setFitToHeight(0);
        ...
        $contador = 0;
        $contador_total_productos = 0;
        $id_product_temp = "producto_1";
        foreach($imagenes as $img){
            ...
            // Cambiamos que la primera imagen sea cover = 1
            if ($id_product_temp != $id_product){
                $id_product_temp = $id_product;
                $imagenes[$contador]["cover"] = 1;
                // GUARDAMOS EN CSV PRODUCTOS
                $objPHPExcel_products->getActiveSheet()
                ->setCellValue('A'.($contador_total_productos+2),$id_product);
                $objPHPExcel_products->getActiveSheet()
                ->setCellValue('M'.($contador_total_productos+2),$reference);
                $contador_total_productos++;
            }else{
                $imagenes[$contador]["cover"] = 0;
            }
            // Chequeamos nombre del color
        ...
        echo "Total productos: ".$contador_total_productos."<br>";
        // Guardamos productos
        $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel_products, 'Excel2007');
        $objWriter->save('productos.xls');
        ...
        foreach($imagenes as $img){
            ...
            $objPHPExcel->getActiveSheet()
                ->setCellValue('M'.$contador,$img["cover"]);     
            $contador++;
        }

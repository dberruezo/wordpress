...
for ($fila=2;$fila<584;$fila++){
	if ($fila != (int)$fila){
		$temp = $objPHPExcel->getActiveSheet()->getCell('E'.$fila)->getValue();
		$temp = trim($temp);
		$temp = strlower($temp);
		$temp = ucfirst($temp);
		array_push($vectorTotalColores,trim($objPHPExcel->getActiveSheet()->getCell('E'.$fila)->getValue()));
	}
	
}

$vector_femenino = array(
	"negra" 	=> "negro",
	"roja"		=> "rojo",
	"blanca"	=> "blanco",
	"negra"		=> "negro",
...
foreach($imagenes as $img){
	// Productos
	$reference	= $img[0]->reference;
	$img_url	= $img[0]->url;
    $id_product = $img[0]->id_product;
	$position   = $img[0]->position;
	$cover		= $img[0]->cover;
	$legend	    = $img[0]->legend;
	$img_url	= $img[0]->url;
    $legend_one = comprobar_color($legend);
	if (!$legend_one){
		$legend_two = $convertir_femenino($legend);
    }
}

function comprobar_color($legend){
	global $vector_colores;
	$findme   = $legend;
	$encontrado = false;
	//foreach (expresión_array as $clave => $valor)
	foreach($vector_colores as $clave_color => $convertir_color){
		$encontrado = false;
		if ($clave_color == $legend){
			$pos = strpos($clave_color, $findme);
...
function convertir_femenino($legend){
	global $vector_femenino;
	$findme   = $legend;
	$encontrado = false;
	//foreach (expresión_array as $clave => $valor)
	foreach($vector_femenino as $clave_color => $convertir_color){
		$encontrado = false;
		if ($clave_color == $legend){
			$pos = strpos($clave_color, $findme);
			// Nótese el uso de ===. Puesto que == simple no funcionará como se espera

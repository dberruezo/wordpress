...
require_once( './wp-load.php' );
		$que="SELECT * from temp_nidos where prestashop_id IN (SELECT distinct id_product from ps_image)";
		$query = $GLOBALS['wpdb']->get_results( $que, OBJECT );
		foreach ($query as $row) {
			$nidosok[$row->nido]=$row->nido;
		}
	...
	if($_GET['nido']!="") {
		...
		$que="SELECT * FROM temp_nidos where nido='".$_GET['nido']."'";
		$query = $GLOBALS['wpdb']->get_results( $que, OBJECT );
		echo "<table>";
		echo "<tr>";
		echo "<th>ID PRESTASHOP</th>";
		echo "<th>NIDO</th>";
		echo "</tr>";
		foreach ($query as $row) {
			echo "<tr>";
			echo "<td>".$row->prestashop_id."</td>";
			echo "<td>".$row->nido."</td>";
			echo "</tr>";
		}
		echo "</table>";

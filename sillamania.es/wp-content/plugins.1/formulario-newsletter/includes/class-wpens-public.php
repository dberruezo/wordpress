...
class Wpens_Public {
	...
	public function wpens_add_newsletter() {
        ...
        $nwscheck  = isset( $_POST['nwscheck'] ) ? sanitize_text_field( $_POST['nwscheck'] ) : '';
		$response = array();
		...
		if( isset($_POST['firstname']) && empty($firstname) ) {
			$response['status'] = '0';
			$response['errmsg'] = __( 'Introduce un nombre', 'wpens' );
		} 

		// Check email validation
		if( empty($email) ) {
			$response['status'] = '0';
			$response['errmsg'] = __( 'Introduce un email válido.', 'wpens' );
		} else if( !filter_var($email, FILTER_VALIDATE_EMAIL) ) {
			$response['status'] = '0';
			$response['errmsg'] = __( 'Introduce un email válido.', 'wpens' );
		} else if( $nwscheck!=1 ) {
			$response['status'] = '0';
			$response['errmsg'] = __( 'Debes aceptar los términos y condiciones.', 'wpens' );
		} else if( count($myrows) > 0 ) {
			$response['status'] = '0';
			$response['errmsg'] = __( 'Gracias por apuntarte!', 'wpens' );
		}
